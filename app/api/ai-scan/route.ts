...
export async function POST(req: NextRequest) {
  try {
    const { imageBase64, mediaType, singleComponent, textOnly, query } = await req.json()
    ...
    if (textOnly && query) {
      const message = await client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `Normaliza este nombre de producto tecnológico al nombre oficial completo con marca y modelo: "${query}".
Responde ÚNICAMENTE con un array JSON de un solo string. Ejemplo: ["NVIDIA GeForce RTX 4090 24GB"]
Si no reconoces el producto, devuelve el nombre tal como está: ["${query}"]`
        }]
      })
