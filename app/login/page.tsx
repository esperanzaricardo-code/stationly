    ...
    <div style={{
      ...
      <div style={{
        ...
        {[
          ...
          <div key={f.label} style={{ marginBottom: 14 }}>
            ...
            <input type={f.type} value={f.value} onChange={e => f.set(e.target.value)} placeholder={f.placeholder}
              ...
              style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--font-body)', fontSize: 14, padding: '11px 14px', borderRadius: 'var(--radius-sm)', outline: 'none', marginBottom: f.label === 'Contraseña' ? 6 : 0 }} />
          </div>
        ))}
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: 'var(--text-dim)' }}>
          <Link href="/" style={{ color: 'var(--tag-text)', textDecoration: 'none' }}>← Volver al feed</Link>
        </p>
      </div>
    </div>
  )
...
export default function LoginPage() {
  return <ThemeProvider><LoginForm /></ThemeProvider>
}
